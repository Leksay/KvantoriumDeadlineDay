...
Shader "Particles/Standard Unlit/MyDisplacement"
{
    Properties
    {
        _MainTex("Albedo", 2D) = "white" {}
        _DispText("DisplacementTexture",2D) = "white"{}
    ...
    Category
    {
        SubShader
        {
            ...
            Pass
            {
                ...
                sampler2D _DispText;
                 half4 fragParticleUnlit2 (VertexOutput IN) : SV_Target
                {
                    float2 tex = IN.texcoord;
                    half4 albedo = readTexture (_MainTex, IN);
                    fixed4 dispVec = tex2D(_DispText,IN.texcoord + _Time.x * 5 - _Time.xy * 0.5);
                    dispVec.x *= - dispVec.y;
                    fixed2 uv_distorted = IN.texcoord + 0.55 * dispVec * _SinTime.a;
                    IN.texcoord = uv_distorted;
                    half4 alb =readTexture (_MainTex, IN);
                    albedo += alb;
                    albedo *= 3.5 * _Color;
